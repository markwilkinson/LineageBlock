module LineageBlock
  # This class Service is the sinatra service

  class Service < Sinatra::Base
    def self.banner
      "lineageblock/#{VERSION} on Ruby #{RUBY_VERSION} (#{RUBY_RELEASE_DATE}) [#{RUBY_PLATFORM}] on Sinatra/#{Sinatra::VERSION} (#{ENV.fetch(
        "RACK_ENV", nil
      )})"
    end

    PUBLIC_FOLDER = "#{LineageBlock.root}/lib/lineageblock/public".freeze
    VIEWS_FOLDER  = "#{LineageBlock.root}/lib/lineageblock/views".freeze
    ...
    def node
      if defined?(@@node)
        ...
        puts "[debug] lineageblock - build (network) node (address: #{LineageBlock.config.address})"
        @@node = Node.new(address: LineageBlock.config.address)
        @@node
      end
    end
